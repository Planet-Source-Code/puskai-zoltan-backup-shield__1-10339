...
For i = intPathLen To 1 Step -1
    If Mid(strPath, i, 1) = "\" Then Exit For
Next
GetFolderName = Mid(strPath, i + 1, intPathLen)
End Function

Public Sub BackupFolder(strSourcePath As String, strDestinationPath As String)
...
MkDir BackupFolderName & "\Backup"
MkDir BackupFolderName & "\Backup\Backup - " & Format(Date, "yyyy.mm.dd")
...
With SHFileOp
    ...
    .pTo = strDestinationPath & "\Backup\Backup - " & Format(Date, "yyyy.mm.dd") & vbNullChar
    .fFlags = lFlags
End With
...
Dim i As Integer
On Error Resume Next
If BackupFolderName = "" Then Exit Sub
If UBound(SourceFolder) = 0 Then Exit Sub
For i = 1 To UBound(SourceFolder)
        BackupFolder SourceFolder(i), BackupFolderName
Next
End Sub
